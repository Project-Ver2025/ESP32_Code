...
// Recording timeouts
unsigned long recordingStartTime = 0;    // millis() when recording started
int RECORDING_TIMEOUT = 5000;  // 5 seconds in ms
...
String askGeminiWithSearch(const String& prompt) {
  ...
  int written = snprintf(payloadBuffer, MAX_PAYLOAD_SIZE,
    "{\"contents\": [{\"parts\": ["
    "{ \"text\": \"%s, use the image if required, answer in less than %d words and also for context I am blind take that into consideration in your response, do not say anything about being blind, use metric units for any distances and I am in Canberra, Australia\" },"
...
String task_classification(String voice_input){
  ...
  // String input_message = "You are a classifier. Your ONLY job is to return one category label in the exact format below.";
  // input_message += "Do not add any explanations, extra words, or punctuation.";
  // input_message += "If the input is unclear, map it to the closest option.";
  // input_message += "User Input: " + voice_input;
  // input_message += " Valid outputs (choose ONE only):";
  // input_message += "1 → Live Task Execution (e.g., 'Tell me when you see a cat')";
  // input_message += "2 → Image Analysis/Question (e.g., 'Describe whats in front of me')";
  // input_message += "3 → General Conversation/Info Retrieval (e.g., 'Whats the weather?')";
  // input_message += "4 → Image Reading (e.g., 'Can you read this sign?')";
  // input_message += "5 → Help (e.g., 'Help', 'No audio data')";
  // input_message += "6, <Speed> <Length> → Response Modification (Speed ∈ {Fast, Normal}, Length ∈ {Short, Long})";
  // input_message += " Examples: '6, Normal Short', '6, Fast Long', '6, Long' (if speed missing, assume Normal)";
  // input_message += "7, <Number> → Text-to-speech speed (Number is multiplier, e.g., '7, 1.5', '7, 0.5')";
  // input_message += "Return ONLY one of the above outputs. Nothing else.";

  ///// V3

  String input_message = "You are a strict classifier. Based on the user input, output ONLY the task number (1–8) ";
  input_message += "and any required modifiers, nothing else. Never add extra text or explanations.";
  input_message += "User input: " + voice_input + " Tasks: ";
  // Task 1
  input_message += "1. Live Task Execution → The user wants an action triggered by an event or condition.";
  input_message += " Examples: 'Tell me when you see a cat' → 1, 'Notify me if the light turns red' → 1";
  // Task 2
  input_message += "2. Image Analysis/Question → The user asks about or requests a description of an image.";
  input_message += " Examples: 'Describe what's in front of me' → 2, 'Is there a laptop in this picture?' → 2";
  // Task 3
  input_message += "3. General Conversation/Information Retrieval → Chat or factual questions.";
  input_message += " Examples: 'What's the weather like?' → 3, 'Tell me a joke' → 3";
  // Task 4
  input_message += "4. Image Reading → The user wants text in an image read aloud or interpreted.";
  input_message += " Examples: 'Read this sign' → 4, 'Does this label have gluten?' → 4";
  // Task 5
  input_message += "5. Help → General assistance, or fallback when unclear.";
  input_message += " Examples: 'Help me' → 5, 'No audio data' → 5";
  // Task 6
  input_message += "6. Response Modification → The user wants to change LLM response speed or length.";
  input_message += " Options: Speed = Fast, Normal | Length = Short, Long";
  input_message += " Examples: 'Make responses fast and long' → 6, Fast Long, 'Short answers please' → 6, Short";
  // Task 7
  input_message += "7. TTS Speed Modification → Adjust text-to-speech playback speed.";
  input_message += " Examples: 'Make the speech slower, 0.5 times speed' → 7, 0.5, 'Make the speaking speed two times' → 7, 2";
  // Task 8
  input_message += "8. Recording Timeout Adjustment → Adjust how long recording continues before auto-stop.";
  input_message += " Value must be in seconds.";
  input_message += " Examples: 'Set recording timeout to 30 seconds' → 8, 30, 'Make timeout 5s' → 8, 5";

  input_message += "Return format examples: '1', '6, Fast Long', '7, 1.5',  '8, 20'";
  input_message += "Output ONLY the number (and modifiers if required). No other text.";
...
void performTask(String task_selected, String transcription) {
  ...
  if (task_selected == "1") {    
    ...
    sendLongStringOverBLE(content + "<EOM>");
      ...
      // TASK 8
      else if (std::regex_match(taskStd, match, re8)) {
        int commaIndex = task_selected.indexOf(',');

        if (commaIndex != -1) {
          String timeoutStr = task_selected.substring(commaIndex + 1);
          timeoutStr.trim();

          int newTimeoutSec = timeoutStr.toInt();  // seconds
          if (newTimeoutSec > 0 && newTimeoutSec <= 120) {  // sanity check, e.g. max 2 min
            RECORDING_TIMEOUT = newTimeoutSec * 1000;  // convert to ms
            Serial.println("Recording timeout updated to: " + String(RECORDING_TIMEOUT) + " ms");
            sendLongStringOverBLE("Recording timeout set to " + String(newTimeoutSec) + " seconds<EOM>");
          } else {
            sendLongStringOverBLE("Invalid timeout value: " + timeoutStr + " (must be 1–120 seconds)<EOM>");
          }
        }
      } 
    }
...
void loop() {  
  ...
  if (isRecording) {
    ...
    // CHECK IF AUDIO RECORDING IS LONGER THAN START TIME THEN STOP
    if (millis() - recordingStartTime >= RECORDING_TIMEOUT && isRecording && receivedCommand.length() == 0) {
      if (millis() - lastStopTime < 1000) {
          Serial.println("Stop command ignored (debounce after timeout)");
          return;
      }
      lastStopTime = millis();
